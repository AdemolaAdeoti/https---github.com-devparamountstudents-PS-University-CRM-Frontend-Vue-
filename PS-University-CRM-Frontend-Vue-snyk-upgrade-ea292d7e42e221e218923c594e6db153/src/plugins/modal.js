...
export function showModal(options) {
 ...
 const { 
  ...
  modal.innerHTML = `
      <div class="cmodal" style="width: 100vw;height: 100vh;position: fixed;left: 0;top: 0;right: 0;bottom: 0;background-color: rgba(0, 0, 0, 0.5);display: flex;align-items: center;justify-content: center;z-index: 100000;">
        <div class="modal-content-paramount bg-white d-flex align-items-center flex-column px-5 zoom-out-bounce" style="max-width:85%;max-heigh:80vh; position:relative;">
        ...
        ${options?.text? "<p class='text-center w-75'>"+ options?.text + "</p>":""}        
          ...
          ${
            options.cancelText || false?
              `<button class="btn-cancel ripple text-center py-2 " style="border-radius: 10px;width:45%;background:${cancelBackgroundColor}; border:1px solid ${cancelOutlineColor}">${
                  options.cancelText || "Cancel"
             }</button>`
            :''
            }
            ${ 
              confirmButton?`<button class="btn-confirm ripple text-center py-2 " style="border-radius: 10px;width:45%;background:${confirmBackgroundColor}; border:1px solid ${confirmOutlineColor}">${
